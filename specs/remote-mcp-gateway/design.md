...
| OAuth トークン保管 | サーバー側 (Firestore) 保持 | 組織コネクタの仕組み上クライアント送信は不可 |
| 状態管理 | Firestore (全状態を外部ストア化) | Cloud Run ステートレス化。再起動・スケールアウトに耐える |
| トークン TTL | Bearer: 1h、Refresh: 7d (独立) | Bearer 短命で漏洩リスク低減、Refresh 長命でユーザー体験維持 |
...
| Firestore 暗号化 | AES-256-GCM アプリケーション層暗号化 | Google OAuth トークン等の機密データを二重暗号化。鍵は Secret Manager に保管 |
...
5. MCP Gateway が Google OAuth トークンを Firestore に保存
...
| OAuth トークン・セッション状態 | Firestore (AES-256-GCM 暗号化、鍵は Secret Manager) | 全状態を外部化。TTL ポリシーで自動クリーンアップ |
